...
zendesk_client = ZendeskClient(
    ...
    oauth_token_path=os.getenv("ZENDESK_OAUTH_TOKEN_PATH", ".zendesk_oauth_tokens.json"),
...
async def handle_list_tools() -> list[types.Tool]:
    ...
    return [
        types.Tool(
            name="begin_oauth_authorization",
            description="Start the one-time Zendesk OAuth authorization flow",
            inputSchema={
                "type": "object",
                "properties": {
                    "scope": {
        ...
        types.Tool(
            ...
            description="Complete Zendesk OAuth authorization after approving access in the browser",
            inputSchema={
                "type": "object",
                "properties": {
                    "code": {"type": "string", "description": "OAuth authorization code"},
                    "state": {"type": "string", "description": "OAuth state returned by Zendesk"}
                },
                "required": ["code", "state"]
            }
        ),
...
async def handle_call_tool(
    ...
    try:
        if name == "begin_oauth_authorization":
            result = zendesk_client.begin_oauth_authorization(
        ...
        elif name == "complete_oauth_authorization":
            if not arguments:
                raise ValueError("Missing arguments")
            result = zendesk_client.complete_oauth_authorization(
                code=arguments["code"],
